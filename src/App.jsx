...
function App() {
  ...
  const play = (col) => {
    ...
    if (!state.gameOver) {
      ...
      if (result === state.player1) {
        ...
        // if no win conditions met, just a normal move, start next players turn
        const nextPlayer =
        ...
        dispatch({
          ...
          newTurn,
          message:
            nextPlayer === 1 ? 'Orange Player Turn' : 'Green Player Turn',
        })
  ...
  return (
    <div className='App'>
      ...
      {/* debug component to open modal for testing */}
      {debug && (
        <>
          <button
            className={styles.redButton}
